...
void LCD_DisplayNumber(uint16_t x, uint16_t y, int number, uint8_t len);                    // 显示整数
...
#define LCD_SPI                 SPI2                                                // LCD用到的是SPI1
#define LCD_SPI_APBClock_Enable RCC_APB1PeriphClockCmd(RCC_APB1Periph_SPI2, ENABLE) // 使能外设时钟

#define LCD_SCK_PIN             GPIO_Pin_13          // SCK引脚， 需要重定义SPI1的IO口复用
#define LCD_SCK_PORT            GPIOB                // SCK引脚用到的端口
#define GPIO_LCD_SCK_CLK        RCC_APB2Periph_GPIOB // SCK引脚IO口时钟

#define LCD_SDA_PIN             GPIO_Pin_15          // SDA引脚， 需要重定义SPI1的IO口复用
#define LCD_SDA_PORT            GPIOB                // SDA引脚用到的端口
#define GPIO_LCD_SDA_CLK        RCC_APB2Periph_GPIOB // SDA引脚IO口时钟

#define LCD_CS_PIN              GPIO_Pin_12          // CS片选引脚，低电平有效
#define LCD_CS_PORT             GPIOB                // CS引脚用到的端口
#define GPIO_LCD_CS_CLK         RCC_APB2Periph_GPIOB // CS引脚IO口时钟

#define LCD_DC_PIN              GPIO_Pin_14          // 数据指令选择  引脚
#define LCD_DC_PORT             GPIOB                // 数据指令选择  GPIO端口
#define GPIO_LCD_DC_CLK         RCC_APB2Periph_GPIOB // 数据指令选择  GPIO时钟

#define LCD_Backlight_PIN       GPIO_Pin_8           // 背光  引脚
#define LCD_Backlight_PORT      GPIOA                // 背光 GPIO端口
#define GPIO_LCD_Backlight_CLK  RCC_APB2Periph_GPIOA // 背光 GPIO时钟
